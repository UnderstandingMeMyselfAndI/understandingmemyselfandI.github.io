...
export default function App({ cryptoKey, onLock }) {
	...
	const toggleFav = (tool) => {
		const newFavs = state.favs.includes(tool) ? state.favs.filter((t) => t !== tool) : [...state.favs, tool]
	...
	return (
		<div>
			...
			{['CBT', 'DBT', 'ACT'].map((tool) => (
				<div key={tool}>
					<label>
						<input type='checkbox' checked={state.favs.includes(tool)} onChange={() => toggleFav(tool)} />
						{tool}
					</label>
				</div>
			))}
			...
			{!confirmClear && <button onClick={() => setConfirmClear(true)}>Clear Local Data</button>}
			{confirmClear && (
				<div>
					<p>Are you sure you want to delete all local data? Synced data remains safe.</p>
